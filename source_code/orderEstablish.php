        ...
        <main id="main" class="">
            <div>
                ...
                foreach ((array)$shoppingCart as $car) {
                    $sqlch = sprintf("SELECT * FROM Car WHERE CarID = '%s'", $car);
                    $datach = mysqli_fetch_assoc(mysqli_query($con, $sqlch));
                    if ($datach['OrderID'] != NULL) {
                        echo "<script>alert('購物車錯誤，訂單未成立');
                ...
                $sql = sprintf("INSERT INTO ORDERS(TotalPrice, OrderDate, PaymentMethod, CouponID, CustomerID, ShipFee, Address) VALUE(%s, NOW(), %s, %s, %s, %s, '%s')", $totalPrice, $paymentMethod, $CouponID, $_SESSION['MemberID'], $shipFee, $address);
                ...
                if (mysqli_query($con, $sql)) {
                    ...
                    $count = 1;
                    foreach ((array)$shoppingCart as $car) {
                        $sqlcar1 = sprintf("UPDATE CAR SET OrderID = %s WHERE CarID = %s", strval($OrderID), $car);
                        $datacar1 = mysqli_query($con, $sqlcar1);
                        if (isset($_POST[sprintf('CouponNo%d', $count)]) && $_POST[sprintf('CouponNo%d', $count)] != '') {
                            $sqlcar2 = sprintf("UPDATE CAR SET CouponID = %s WHERE CarID = %s", $_POST[sprintf('CouponNo%d', $count)], $car);
                            $datacar2 = mysqli_query($con, $sqlcar2);
                        }
                        $count += 1;
                    }
                    $_SESSION['shoppingCart'] = array();
                    echo "訂單成立!<br>";
                    echo "訂單編號為!" . strval($OrderID) . "<br>";
                    echo "<a href='user.php'>點此查看訂單</a>";
                } else {
                    echo "Error creating table: " . mysqli_error($con) . '<br>';
                    echo "<a href='shoppingCart.php'>點此返回購物車頁面</a>";
                }
                ?>
            </div>
