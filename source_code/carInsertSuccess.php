...
if (
    !isset($_POST['Brand']) || !isset($_POST['Model']) || !isset($_POST['Age']) || !isset($_POST['Price']) ||
    !isset($_POST['Location']) || !isset($_POST['Distance']) || !isset($_POST['Image'])
) {
    echo '請輸入完整車輛資訊';
?>
    <form action="./carInsert.php">
    ...
    <div id="wrapper">
        ...
        <main id="main" class="">
            <div>
                ...
                $Image    = $_POST['Image'];
                $Note     = $_POST['Note'];
                $sql = sprintf('INSERT INTO CAR(Brand, Model, Age, Price, Location, Distance, LaunchDate, Image, Note, StaffID) 
                VALUES("%s", "%s", %d, %d, "%s", %d, NOW(), "%s", "%s" ,%d)', $Brand, $Model, (int)$Age, (int)$Price, $Location, $Distance, $Image, $Note, $StaffID);
            ...
            <div>
                <div style="margin: 50px 50% 0 8%;">
                    ...
                    ?>
                        <h3 style="margin: 60px 50% 0 8%;">車輛上架上架成功</h3>
                    <?php
                        $new_id = mysqli_insert_id($con);
                        echo "新增後的車輛id為 {$new_id} ";
                    } elseif (mysqli_affected_rows($con) == 0) {
                    ?>
                        <h3 style="margin: 60px 50% 0 8%;">車輛上架失敗</h3>
                        <span>無資料新增</span>
                    <?php
                    } else {
                    ?>
            ...
            <div style="margin: 50px 50% 0 8%;">
                <a href="./carInsert.php">點此返回上架頁面</a>
            </div>
            <?php
            function function_alert($message)
            {
                echo "<script>alert('$message');window.location.href = 'carInsert.php';</script>";
                return false;
            }
            ?>
        </main>
