...
if (
    !isset($_POST['CouponType']) || !isset($_POST['StartTime']) || !isset($_POST['EndTime']) || !isset($_POST['Discount']) || !isset($_POST['UsageLimit'])
) {
    echo '請輸入完整優惠券資訊';
?>
    <form action="./couponInsert.php">
        ...
        <main id="main" class="">
            <div>
                ...
                <div style="margin: 50px 50% 0 8%;">
                    ...
                    $sql = sprintf('INSERT INTO COUPON(CouponType, StartTime, EndTime, UsageLimit, Discount, Descriptions, StaffID) 
                                    VALUES(%s, "%s", "%s", %s, %s, "%s" ,%s)', $CouponType, $StartTime, $EndTime, $UsageLimit, $Discount, $Descriptions, $StaffID);
                    ...
                    ?>
                        <h3 style="margin: 60px 50% 0 8%;">優惠券上架成功</h3>
                    <?php
                        $new_id = mysqli_insert_id($con);
                        echo "新增後的優惠券id為 {$new_id} ";
                    } elseif (mysqli_affected_rows($con) == 0) {
                    ?>
                        <h3 style="margin: 60px 50% 0 8%;">優惠券上架失敗</h3>
                        <span>無資料新增</span>
                    <?php
                    } else {
                    ?>
            ...
            <div style="margin: 50px 50% 0 8%;">
                <a href="./couponInsert.php">點此返回上架頁面</a>
            </div>
            <?php
            function function_alert($message)
            {
                echo "<script>alert('$message');window.location.href = 'couponInsert.php';</script>";
                return false;
            }
            ?>
